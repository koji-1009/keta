...
void main() {
  test(
    'a top-level \$ref absent from deps is a schema-authoring StateError, '
    'not a violation',
    () {
  ...
  group('type integer', () {
    ...
    test(
      'a zero-fraction double (1.0) is rejected — a deliberate deviation '
  ...
  group('oneOf', () {
    ...
    test(
      'a discriminator missing "propertyName" is a StateError naming that '
      'key',
      () {
    ...
    test(
      'a non-Map discriminator is a StateError, not a TypeError crash',
      () {
    ...
    test(
      'a non-Map discriminator.mapping is a StateError, not a TypeError '
      'crash',
      () {
    ...
    test(
      'a mapped ref absent from deps is a schema-authoring StateError, not '
    ...
    test(
      'oneOf present but not a list is a schema-authoring StateError',
      () {
  ...
  group('enum on non-string types', () {
    ...
    test(
      '"enum" that is not a list at all is a schema-authoring StateError, '
      'not a silent pass',
      () {
  ...
  group('additionalProperties', () {
    ...
    test(
      'a value that is neither false nor an object schema (e.g. true) is a '
        ...
        expect(
          ...
          throwsA(
            isA<StateError>().having(
              (e) => e.message,
              'message',
              allOf(contains('"BadAdditional"'), contains('additionalProperties')),
            ),
          ),
        );
      },
  ...
  group('unknown schema type', () {
    test(
      'a typo\'d type is a schema-authoring StateError, not a violation or '
      'a silent pass',
      () {
    ...
    test(
      'a JSON Schema type array is out of the canonical subset — a '
  ...
  group(
    'malformed schema fragments throw a descriptive StateError, never a '
    ...
    () {
      ...
      test(
        'a "required" list with a non-string entry is caught too, not '
        'silently dropped',
        () {
      ...
      test(
        'a non-map property sub-schema names the schema and the property',
        () {
