...
void main() {
  group('enforceSecurity', () {
    ...
    App<Env> build(
      SecurityPolicy<Env> policy,
      void Function(App<Env>) routes,
    ) {
      final app = App<Env>()
        ..use(recover())
    ...
    test('a public route (no declaration, empty default) is admitted', () async {
      final app = build(
    ...
    test('a declared route rejects a missing token and admits a valid one', () async {
      final app = build(
    ...
    test('an explicitly public route ([]) overrides a non-empty default', () async {
      final app = build(
    ...
    test('with a non-empty default an unmatched path is 401, not 404 '
        '(secure-by-default auth wall); an authenticated one reveals the 404', () async {
      final app = build(
        SecurityPolicy(defaults: const [bearer], verifiers: verifiers),
        (a) => a.get('/x', (c) => c.text('ok')),
      );
