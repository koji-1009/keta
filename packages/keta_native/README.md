...
3. Emits the result in one of two modes. With linking enabled (AOT `dart build`), the compile produces a static archive routed to the in-package link hook (`hook/link.dart`), which tree-shakes it to exactly the symbols the `@Native` bindings reference — the keep-list lives in `hook/symbols.dart`, and a test fails if it drifts from the bindings — so the deployed library carries only the BoringSSL code reachable from the bound symbols. Without link hooks (JIT: `dart test` / `dart run`), the full `libcrypto` ships as the dynamic library.
