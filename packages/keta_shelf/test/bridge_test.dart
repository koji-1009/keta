/// The keta<->shelf bridge: directional translation of requests/responses
/// (`ketaToShelf`, `shelfToKeta`), body framing and size limits, remote
/// address extraction, and refusal of what the bridge cannot carry
/// (protocol upgrades). `Host`-header/URI reconstruction has its own file,
/// host_uri_test.dart.
library;
...
void main() {
  ...
  group('framing and streaming', () {
    ...
    // DUPLICATE removed: "shelfToKeta reflects the Host header into
    // requestedUri" pinned the same Host->requestedUri reflection already
    // covered, more strongly (it also asserts the 200 status), by
    // host_uri_test.dart's "passes a valid host:port Host header through
    // unchanged" — same route shape, same header, no distinct code path.
