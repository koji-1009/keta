...
void main() {
  test('concurrent transactions serialize — no nesting error, no lost updates',
      () async {
    final db = SqliteDb.memory();
    addTearDown(db.close);
    await db.writer.execute('create table counter (id integer primary key, n integer)');
  ...
  test('a plain write concurrent with an open transaction does not interleave',
      () async {
    final db = SqliteDb.memory();
    addTearDown(db.close);
    ...
    final tx = db.transaction<int>((c) async {
