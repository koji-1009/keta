...
/// place it is named. There is no `captures` declaration because `id` is a
/// string, which is what a capture is unless the file says otherwise.

const getDoc = RouteDoc(response: userDtoSchema, summary: 'Fetch a user');
...
const putDoc = RouteDoc(
  requestBody: userDtoSchema,
  response: userDtoSchema,
  summary: 'Replace a user',
);

Future<Response> put(Context<Env> c) async {
...
const deleteDoc = RouteDoc(summary: 'Delete a user');

Future<Response> delete(Context<Env> c) async {
  final changed = await c.get(txConn).execute(
